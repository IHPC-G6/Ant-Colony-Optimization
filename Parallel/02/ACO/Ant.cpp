...
Ant::Ant(ACO* aco, ACOGraph* g) {
    ...
    pheromone_delta_matrix.resize(graph->n * graph->n, 0.0);
    ...
    eta.resize(graph->n * graph->n, 0.0);
    // fill eta array with 1.0 / d_{ij} except the diagonal
    for (int i = 0; i < graph->n; i++) {
        for (int j = 0; j < graph->n; j++) {
            if (i != j) { 
                eta[i * graph->n + j] = 1.0 / graph->cost_matrix[i * graph->n + j]; 
            }
        }
    }
...
int Ant::select_next_node() {
    ...
    for (int i : allowed_nodes) {
        denominator += pow(graph->pheromone_matrix[current_node * graph->n + i], colony->alpha) * pow(eta[current_node * graph->n + i], colony->beta);
    }

    std::vector<double> probabilities(graph->n, 0);
    for (int i = 0; i < graph->n; ++i) {
        if (std::find(allowed_nodes.begin(), allowed_nodes.end(), i) != allowed_nodes.end()) {
            probabilities[i] = (pow(graph->pheromone_matrix[current_node * graph->n + i], colony->alpha) * pow(eta[current_node * graph->n + i], colony->beta)) / denominator;
        }
    }
    ...
    solution_cost += graph->cost_matrix[current_node * graph->n + selected_node];
...
void Ant::update_pheromone_delta_matrix() {
    ...
    for (int ii = 1; ii < solution.size(); ++ii) {
        ...
        if (colony->update_strategy == 0) { // ant-cycle system: // wikipedia 
            pheromone_delta_matrix[i * graph->n + j] = colony->Q / solution_cost;
        }
        else if (colony->update_strategy == 1) { // ant-quality system
            pheromone_delta_matrix[i * graph->n + j] = colony->Q;
        }
        else if (colony->update_strategy == 2) { // ant-density system
            pheromone_delta_matrix[i * graph->n + j] = colony->Q / graph->cost_matrix[i * graph->n + j];
        }
    }
}